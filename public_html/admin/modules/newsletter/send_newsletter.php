...
$custom_headtags = '
		<!-- TinyMCE -->
		<script type="text/javascript" src="/admin/js/newsletter_init.js"></script>
		<!-- /TinyMCE -->
		';
...
if (!$authorized) {
	...
	if ($num > 0)  { // If it ran OK, display the records.
		...
		if (isset($_POST['user']) && $_POST['submit'] == "Send Mail") {
			...
			$query = "
					...
					FROM newsletter AS n,customers AS c
					WHERE n.newsletter_id=c.newsletter_id
					AND n.newsletter_id IN (";
			foreach ($_POST['user'] as $key => $value) {
				$query .= $value . ',';
			}
			$query = substr ($query, 0, -1) . ")
					ORDER BY name ASC"; // Standard query
			...
			echo '<p style="text-align:center;"><a id="select_all" href="#" style="margin-right:50px;">Select All</a><input type="submit" name="submit" value="Send Mail" /></p>';
