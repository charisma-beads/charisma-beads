...
if (!$authorized) {
	...
	print "<tr><td height=\"25\" class=\"Link\" onMouseOver=\"this.className='bodyLink'\" onMouseOut=\"this.className='Link'\"><a href=\"discontinued_cat_list.php\" class=\"Link\">Discontinued Categories</a></td></tr>";
    ...
    if ($num > 0) { // If it ran OK, display the records.
        ...
        foreach ($data as $key => $row) {
            ...
            if ($row['enabled'] == 1) {
            	$status = "<a href=\"{$_SERVER['PHP_SELF']}?status=0&pcid={$row['category_id']}";
            	if (isset ($_GET['s']) && isset ($_GET['np'])) {
            		$status .= "&s={$_GET['s']}&np={$_GET['np']}";
            	}
            	$status .= "\" ><img src=\"/admin/images/on.png\" alt=\"Click to disable Category\"/></a>";
            } else {
            	$status = "<a href=\"{$_SERVER['PHP_SELF']}?status=1&pcid={$row['category_id']}";
            	if (isset ($_GET['s']) && isset ($_GET['np'])) {
            		$status .= "&s={$_GET['s']}&np={$_GET['np']}";
            	}
            	$status .= "\" ><img src=\"/admin/images/off.png\" alt=\"Click to enable Category\"/></a>";
            }
