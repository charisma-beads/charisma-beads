...
// Number of records to show.
    ...
    if ($num > 0) { // If it ran OK, display the records.
        ...
        if ($num_pages > 1) {
            ...
            if ($current_page != 1){
                $content .= '<a href="news.php?s=' . ($start - $display) . '&np=' . $num_pages . '">Previous</a> ';
            }
            
            // Make all the numbered pages.
            for ($i = 1; $i <= $num_pages; $i++) {
                if ($i != $current_page) {
                    $content .= '<a href="news.php?s=' . (($display * ($i - 1))) . '&np=' . $num_pages . '">' . $i . '</a> ';
                } else {
                    $content .= $i . ' ';
                }
            }
        ...
        if ($current_page != $num_pages) {
            $content .= '<a href="news.php?s=' . ($start + $display) . '&np=' . $num_pages . '">Next</a> ';
        ...
        // Retrieve and $content .= every record.
	while ($row = mysql_fetch_array ($result)) {
		$content .= "<div align=\"center\">" . "<h1 style=\"text-align:center;\">{$row['title']}</h1>
		<p><b>{$row['date']}</b></p>" . $row['entry'] . "<hr style=\"width:50%; height:6px; text-align:center; border:3px ridge skyblue;\"></div>\r\n";
        }
