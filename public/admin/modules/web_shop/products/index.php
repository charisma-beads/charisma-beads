    ...
    if ($num > 0) { // If it ran OK, display the records.
        ...
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) { 
			...
			if ($row['enabled'] == 1) {
				...
				$status .= "\" ><img src=\"/admin/images/on.png\" alt=\"Click to disable Product\"/></a>";
			} else {
				$status = "<a href=\"{$_SERVER['PHP_SELF']}?status=1&pid={$row['product_id']}";
				if (isset ($_GET['s']) && isset ($_GET['np'])) {
					$status .= "&s={$_GET['s']}&np={$_GET['np']}";
				}
				$status .= "\" ><img src=\"/admin/images/off.png\" alt=\"Click to enable Product\"/></a>";
			}
			...
			if (file_exists($_SERVER['DOCUMENT_ROOT'] . $line_img)) {
            ...
            $oos = ($row['quantity'] == 0) ? "<div class=\"oos\"><img src=\"/admin/images/oos.png\" /></div>" : "";
			
            print "<tr bgcolor=\"#$bg\">\n";
			print "<td>$status</td>\n";
			print "<td align=\"left\">{$row['product_name']}</td>\n";
			print "<td align=\"left\">$oos{$row['short_description']}</td>\n";
