...
var SaveList = function(element,prevElement,insertType) {
	var query = 'el=' + element + '&prev=' + prevElement + '&insert_type=' + insertType;
...
var waitSign = function() {
	$('loading').setText('Please wait while I update the database');
	$('loading').setStyles({
		'display': 'block',
		'width': '400px'
	});
...
window.addEvent('domready', function() {
	sortIt = new Nested('categories', {
		...
		onComplete: function(el) {
			...
			if (el.getPrevious()) {
				var prevElement = el.getPrevious().id;
				var insertType = "after child";
			} else {
				var elementParent = el.getParent();
				var prevElement = elementParent.getParent().id;
				var insertType = "new child";
			}
			
			SaveList(element,prevElement, insertType);
		}
	});
});
